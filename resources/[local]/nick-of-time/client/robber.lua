...
CreateThread(function()
    while true do
        ...
        if not isRobber() then
            ...
            elseif house then
                -- One thing happens at a door now: the bag goes through it.
                -- He no longer gets to end the round from in here - the clock
                -- does that, and what it finds when it arrives is the game
                -- (Darren's rewrite; see Config.round.SUDDEN_S).
                help(('~INPUT_PICKUP~ Stash %s ~w~- the only money that counts')
                    :format(NickHUD.money(purse.carried or 0)))
